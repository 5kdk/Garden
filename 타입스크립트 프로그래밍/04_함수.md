...
- 이제 두 가지 방식으로 reserve를 호출할 수 있으므로 reserve를 구현할 때 타입스크립트에 reserve가 어떤 방식으로 호출되는지 확인시켜 주어야 한다.
  ...
  let reserve: Reserve = (
    ...
    if (toOrDestination instanceof Date && destination !== undifined) {
      // 편도 여행 예약
...
- 타입스크립트 프로그램을 구현할 때는 먼저 함수의 타입 시그니처를 정희한 다음 구현을 추가한다. 구현을 시작하기 전에 프로그램을 타입 수준에서 구상해보면 모든 것이 이치에 맞는지를 상위 수준에서 확인할 수 있다.
