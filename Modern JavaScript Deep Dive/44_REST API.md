...
REST API는 자원(_resource_), 행위(_verb_), 표현(_representation_)의 3가지 요소로 구성된다. REST는 자체 표현 구조(_self-descriptiveness_)로 구성되어 REST API만으로 HTTP 요청의 내용을 이해할 수 있다.
...
특정 리소스 전체를 교체할 때 사용한다. 다음 예제에서는 todos 리소스에서 id로 todo를 특정하여 id를 제외한 리소스 전체를 교체한다. PUT 요청 시에는 `setRequestHeader`메서드를 사용하여 요청 몸체에 담아 서버로 전송할 페이로드의 MIME 타입을 지정해야 한다.
